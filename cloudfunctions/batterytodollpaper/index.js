...
exports.main = async (event, context) => {
  ...
  // 获取当前积分数
  let resp = await got(GATEWAY + '/batterypoint/get/' + wxContext.OPENID, {
    method: 'GET',
    headers: {
      accept: "*/*"
    }
  ...
  // 更新积分数据
  let pro_change = got(GATEWAY + '/batterypoint/change/' + wxContext.OPENID + '/' + new_number, {
    method: 'PUT',
    headers: {
      accept: "*/*"
    }
  ...
  for (let i = 0; i < rule.get * cost_light; i++) {
    let pro_item = got(GATEWAY + '/dollpaper/create', {
      method: 'POST',
      headers: {
        accept: "*/*"
      },
  ...
  // 获取当前全部娃娃券
  resp = await got(GATEWAY + '/dollpaper/get/' + wxContext.OPENID, {
    method: 'GET',
    headers: {
      accept: "*/*"
    },
